...
const AdminRoles = () => {
  ...
  const handleCreateRole = async () => {
    if (!newRoleName) return alert("Ingresa el nombre del rol");
    try {
      await createAdminRole({ nombre_rol: newRoleName, descripcion: newRoleDesc });
      setNewRoleName("");
      setNewRoleDesc("");
      fetchRoles();
    } catch (error) {
  ...
  const handleUpdateRole = async (id_rol) => {
    ...
    if (!role) return;
    try {
      await updateAdminRole(id_rol, { nombre_rol: role.nombre_rol, descripcion: role.descripcion });
      fetchRoles();
    } catch (error) {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="3"
                  className="w-full border-2 border-gray-200 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none resize-none"
                />
              </div>
              
              <button
                onClick={handleCreateRole}
                className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-6 py-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-2"
              >
