...
export default function ProjectCard({ project, onPublish, onEdit, onDelete, onView, onReleasePayment }) {
    ...
    return (
        <div className="relative bg-white rounded-2xl p-6 shadow-md hover:shadow-2xl transition-all duration-300 border border-gray-100 group overflow-hidden">
            ...
            {/* Botón Completar y Liberar Pago */}
            {project.estado_publicacion === "activo" && (
                <button
                    onClick={() => onReleasePayment(project)}
                    className="absolute top-4 right-4 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all duration-200 shadow-md hover:shadow-xl hover:scale-105 text-sm z-10 flex items-center gap-2"
                >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
